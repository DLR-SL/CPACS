...
def section_xml(uid, name, airfoil_uid, section_translation, element_rotation, element_scaling):
    ...
    lines.append(indent(transformation_xml(translation=section_translation), 1))
    lines += [
        "    <elements>",
    ...
    lines.append(indent(transformation_xml(rotation=element_rotation, scaling=element_scaling), 3))
...
def wing_xml():
    ...
    lines.append(indent(transformation_xml(), 1))
...
def write_example(fits):
    airfoils = "\n".join(
        wing_airfoil_xml(code, *fits[code][:3], description) for code, description in EXAMPLE_AIRFOILS
    )
    write_cpacs_file(
        EXAMPLE_FILE,
        name="Wing with CST airfoils",
        description="Wing airfoils defined by the class-shape transformation (CST) and a wing using them. "
        "Generated by documentation/scripts/cst2D.py.",
        model_uid="CSTAircraft",
        model_name="CST example",
